...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double dx = _x.x() - m_center.x();
		double dy = _x.y() - m_center.y();
		double dz = _x.z() - m_center.z();
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double dy = _x.y() - m_center.y();
		double dz = _x.z() - m_center.z();
		return pow((dx*dx + dy*dy + dz*dz + m_radius*m_radius - m_a*m_a),2) - (4.0 * m_radius*m_radius* (dx*dx+ dy*dy));
	}
